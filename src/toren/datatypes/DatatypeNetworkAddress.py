...
class DatatypeNetworkAddress(Datatype):
  ...
  def _CSharp_to_(self, *args)-> str:
    ...
    setval = f'(long)BitConverter.ToUInt32({objname}.{propertyname}.GetAddressBytes())' # Array.Reverse?
    return setval
  ...
  def Java_Datalayer_Dependencies(self, *args) -> list:
    return ["import com.google.common.net.InetAddresses;"]
  ...
  def _Java_to_(self, *args)-> str:
    ...
    objname = str(argt[1])
    propertyname = str(argt[2])
    setval = f'statement.setLong({indx}, (long) InetAddresses.coerceToInteger({objname}.get{propertyname}()));'
    return setval
