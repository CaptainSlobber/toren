...
class DatatypeInt(DatatypeNumeric):
  ...
  def Python_from_PostgreSQL(self, *args) -> str:
    if self.hasHigherDimensionality():
      return f"np.array(json.loads(bytes({args[0]}).decode('utf-8')), dtype=np.int32)"
    else:
      return f"int({args[0]})"
  ...
  def CSharp_from_(self, *args) -> str:
    ...
    if self.hasHigherDimensionality():
      commas = ","*(len(self.Dimensinality)-1)  
      datatype = f"int[{commas}]" #multidimensional array
  ...
  def _Java_to_(self, *args)-> str:
    ...
    objname = str(argt[1])
    propertyname = str(argt[2])
    setval = f'statement.setInt({indx}, {objname}.get{propertyname}());'
    if self.hasHigherDimensionality():
  ...
  def Java_from_(self, *args) -> str:
    ...
    if self.hasHigherDimensionality():
      ...
      return f'resultset.getInt("{argt[0]}")'