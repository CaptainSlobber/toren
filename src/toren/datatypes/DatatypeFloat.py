...
class DatatypeFloat(DatatypeNumeric):
  ...
  def Python_from_Oracle(self, *args) -> str:
    if self.hasHigherDimensionality():
      return f"np.array(json.loads({args[0]}.read().decode('utf-8')), dtype=np.float32)"
    else:
      return f"float({args[0]})"
  
  def Python_from_MicrosoftSQL(self, *args) -> str:
    if self.hasHigherDimensionality():
      return f"np.array(json.loads({args[0]}.decode('utf-8')), dtype=np.float32)"
    else:
      return f"float({args[0]})"
  
  def Python_from_PostgreSQL(self, *args) -> str:
    if self.hasHigherDimensionality():
      return f"np.array(json.loads(bytes({args[0]}).decode('utf-8')), dtype=np.float32)"
    else:
      return f"float({args[0]})"
  ...
  def _Java_to_(self, *args)-> str:
    ...
    objname = str(argt[1])
    propertyname = str(argt[2])
    setval = f'statement.setFloat({indx}, {objname}.get{propertyname}());'
    if self.hasHigherDimensionality():
  ...
  def Java_from_(self, *args) -> str:
    ...
    if self.hasHigherDimensionality():
      ...
      return f'resultset.getFloat("{argt[0]}")'